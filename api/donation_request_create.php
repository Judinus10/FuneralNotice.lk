...
try {
    ...
    if ($fullName === '' || $email === '' || $phone === '' || $country === '') {
        echo json_encode(['ok' => false, 'message' => 'Please fill all required fields to request a donation.']);
        exit;
    }
    ...
    $stmt = db()->prepare("
        INSERT INTO donation_requests
            (post_id, full_name, email, phone, country, amount, currency, preferred_contact_time, message)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([
        $postId,
    ...
    echo json_encode([
        'ok' => true,
        'message' => 'Thank you. Our team will contact you with bank details to complete your donation.'
    ]);

} catch (Throwable $e) {
    echo json_encode(['ok' => false, 'message' => 'Failed to submit donation request.']);
}